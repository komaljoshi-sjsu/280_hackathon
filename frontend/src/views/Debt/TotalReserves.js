...
function TotalReserves(props) {
    ...
    useEffect(()=> {
        axios.get('http://localhost:5000/debt/getFileData/'+startDate+'/'+endDate+'/totalReserves'+'/'+country).then(res => {
            if(res.status==200) {
                let recs = res.data;
                let yearArr = [];
                let valArr = [];
        ...
        <>
          ...
          <Card>
            <Card.Body>
              <Row>
                <Col md={8}>
                  <label style={{ "font-weight": "bold" }}>Total reserves (% of total external debt)</label>
                  <Chart
                    chartType="LineChart"
                    data={graphData}
                    width="100%"
