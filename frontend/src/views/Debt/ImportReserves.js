...
function ImportReserves(props) {
    ...
    useEffect(()=> {
        axios.get('http://localhost:5000/debt/getFileData/'+startDate+'/'+endDate+'/importReserves'+'/'+country).then(res => {
            if(res.status==200) {
                let recs = res.data;
                let yearArr = [];
                let valArr = [];
        ...
        <>
          ...
          <Card>
            <Card.Body>
              <Row>
                <Col md={8}>
                  <label style={{ "font-weight": "bold" }}>Total reserves in months of imports</label>
                  <Chart
                    chartType="LineChart"
                    data={graphData}
                    width="100%"
